...
```mermaid
flowchart TD
    A["SKILL.md.tmpl<br>人間の書いた手順・プロンプト"] --> C{"gen-skill-docs.ts<br>ビルドスクリプト"}
    B["Source Code<br>メタデータ: コマンド一覧・フラグなど"] --> C
    C -->|マージ・検証| D["SKILL.md<br>AIエージェントが実行時に読み込む"]
...
graph TD
    subgraph "Workspace 1 (e.g. Feature A)"
        C1["Claude Code 1<br>Skill: /office-hours"] <--> S1["gstack Server<br>Port: 12345"] <--> B1[Chromium]
    end

    subgraph "Workspace 2 (e.g. Bugfix B)"
        C2["Claude Code 2<br>Skill: /qa"] <--> S2["gstack Server<br>Port: 45678"] <--> B2[Chromium]
    end

    subgraph "Workspace 3 (Cross-AI Coordination)"
        C3["Claude Code 3<br>Skill: /pair-agent"] <--> S3["gstack Server<br>Port: 34567"]
        Codex[OpenAI Codex CLI] <--> S3
        S3 <--> B3["Chromium<br>(Shared Browser)"]
        B3 --- T1["Tab: Claude"]
        B3 --- T2["Tab: Codex"]
    end

    Conductor["Conductor<br>(Orchestrator)"] -.-> C1
    Conductor -.-> C2
    Conductor -.-> C3
```
